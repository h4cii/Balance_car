...
- USART1 Type-C 串口接收语音上位机的移动和速度命令。
...
板载 Type-C 转串口连接 USART1，串口参数为 `115200 8N1`。它兼容
`voice2uart` 发出的单字节命令 `A/E/B/F/Z/X/Y`，分别用于前进、后退、
右转、左转、停止、快速档和慢速档。

USART1 收到的 `N/O/P/M` 或其他未知字节会被忽略，不会改变模式或触发停车。
USART1 与 USART3 共用控制目标，最后收到的有效移动或速度命令生效。
